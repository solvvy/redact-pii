defineTest('index.js', function (Redactor) {
  ...
  it('should return non-strings', function () {
    let obj = {foo: 'bar'};
    return redactor.redact(undefined).then(res => {
  ...
  it('should redact PII', function () {
    ...
    let expected = 'Hey it\'s PERSON_NAME with ACME Corp. Give me a call at PHONE_NUMBER';
  ...
  it('should replace names', function () {
    return redactor.redact('blah blah\nThis is very important.').then(res => {
      ...
      return redactor.redact('to hearing from you.\n\nAll the best,\n\nAngel\nCustomer Experience\nwww.foo.com').then(res => {
        assert.equal(res, 'to hearing from you.\n\nAll the best,\n\nPERSON_NAME\nCustomer Experience\nwww.foo.com');
      });
    }).then(() => {
      return redactor.redact('getting this sorted out.\n\nKindest regards,\n\nFoo Bar\nCustomer Experience').then(res => {
  ...
  it('should replace credit card numbers', function () {
    return redactor.redact('my card: 1234 5678 8765 4321.').then(res => {
  ...
  it('should replace phone numbers', function () {
    return redactor.redact('my phone: (+44) (555)123-1234.').then(res => {
  ...
  it('should replace ip addresses', function () {
    return redactor.redact('my ip: 10.1.1.235.').then(res => {
  ...
  it('should replace email addresses', function () {
    return redactor.redact('my email: joe123@solvvy.co.uk.').then(res => {
  ...
  it('should replace usernames and passwords', function () {
    return redactor.redact('here is my username: foobar and my password: baz123').then(res => {
  ...
  it('should replace URLs', function () {
    return redactor.redact('My homepage is http://example.com').then(res => {
      ...
      return redactor.redact('complex http://user@pass:example.com:8080/reset/password/12345?foo=bar&hi=there#/app works?').then(res => {
