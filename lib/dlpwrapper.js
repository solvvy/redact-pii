...
module.exports = {
  redactText: function (redactObject) {
    if (!dlp) {
      dlp = new DLP.DlpServiceClient({
          credentials: redactObject.gauth_credentials
          // keyFilename: redactObject.keyFilename
      });
    }
    return Promise.race([dlp.inspectContent({
      ...
      item: {value: redactObject.originalText},
    }).then(response => {
      ...
      if (findings.length > 0) {
        findings.forEach(finding => {
          ...
          redactObject.originalText = redactObject.originalText.replace(re, finding.infoType.name);
        });
      }
      return redactObject.originalText;
    }), new Promise(resolve => setTimeout(resolve, SERVICE_TIMEOUT, redactObject.originalText))
    ]).catch(err => {
