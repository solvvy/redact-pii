...
function DlpRedactor(userOpts) {
  const dlpOptions = (userOpts || {}).googleCloudDLPOptions || {};

  if (!dlp) {
    dlp = new DLP.DlpServiceClient(dlpOptions.clientOptions);
  }
  ...
  return {
    redactText: function(originalText) {
      ...
      let dlpError = false;
      const overrides = dlpOptions.inspectOverrides || {};

      // handle info type excludes and includes
      const infoTypes = defaultInfoTypes
        .filter(function(infoType) {
          return !overrides.excludeInfoTypes || overrides.excludeInfoTypes.indexOf(infoType.name) === -1;
        })
        .concat(
          (overrides.includeInfoTypes || []).map(function(infoTypeName) {
            return { name: infoTypeName };
          })
        );
      ...
      return projectIdPromise.then(function(projectId) {
        return Promise.race([
          ...
          if (dlpOptions.disableFallbackRedaction) {
            throw err;
          } else {
            return originalText;
          }
