...
const dlpWrapper = require('./dlpwrapper.js');

function defaultReplace(name) {
  return name === 'names' ? '$1PERSON_NAME' :
    name === 'greetOrClose' ? 'PERSON_NAME' :
...
function Redactor(userOpts) {
  ...
  return {
    redact: function (text) {
      ...
      return Promise.resolve(text).then(redactedText => {
          ...
          if (name === 'greetOrClose') {
            ...
            while (mtch !== null) {
              ...
              if (mtch2 !== null && mtch2.index === pattern.lastIndex) {
                ...
                redactedText = redactedText.slice(0, mtch2.index) + replacements[name] + suffix +
        ...
        return userOpts && userOpts.enableOnline ? {originalText: redactedText, gauth_credentials: userOpts.gauth_credentials} : redactedText;
      }).then(userOpts && userOpts.enableOnline ? dlpWrapper.redactText : identity);
    }
  };
}
